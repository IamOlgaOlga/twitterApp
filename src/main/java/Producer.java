		...
		for(;;) {
			...
			switch (inputCommand) {
				...
				case (STATISTICS_INDICATOR):
					...
					System.out.print("Tags in the last week: " + tagsList.get(0));
					...
					System.out.println("The most popular tag in the last week: " + tagsList.get(0));
					break;
				default:
					String[] tags = inputCommand.split("[\\s+\\p{Punct}]+");
					for (String tag: tags) {
						try {
							queueTags.put(tag);
						} catch (InterruptedException e) {
